...
import type { ReviewDisplay, ReviewsSummary } from "@/lib/reviews/types";
import { containerClassName, sectionClassName } from "@/lib/layout/classes";

type ReviewsSectionClientProps = {
        ...
        <div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between">
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-[0.32em] text-gold">
              {summary.source} Bewertungen
            </p>
            <p className="mt-4 text-5xl font-semibold tracking-[-0.06em] text-foreground">
              {summary.averageRating.toFixed(1)}
            </p>
            <StarRating className="mt-3" rating={summary.averageRating} />
            <p className="mt-4 text-sm text-muted">
              Basierend auf über {summary.reviewCount}+ Bewertungen in Celle
            </p>
            <LuxuryButton
